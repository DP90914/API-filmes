...
let page = 1
let film_input = document.getElementById("film_input")
...
async function getSearchlist() {
    const url = `https://api.themoviedb.org/3/search/movie?query=${film_input.value}&api_key=${apiKey}&language=pt-BR&page=1`
    const response = await fetch(url)
    const data = await response.json()
    console.log(data)
    return data
...
async function createMovieList() {
    ...
    images.results.forEach(item => {
        ...
        container2.addEventListener('click', async function () {
            ...
            avaliacao_filme.textContent = `${parseInt(data['vote_average'])} estrelas`
...
async function createSearchList() {
    const images = await getSearchlist()
...
btn_pesquisar.addEventListener("click", async function () {
    const container = document.getElementById('main_index')
    container.textContent = ''
    createSearchList()
})
